...
{
	// zamknij wszystkie gniazna klientów i serwera
	for (auto it = this->descrs.begin(); it != this->descrs.end(); it++)
	{
		...
		closesocket(it->fd); // zamknij gniazdo
	}
	...
	while (true) // główna pętla serwera
	{
		...
		while (it != this->descrs.end()) // przejdź przez wszystkich klientów
		{
			if (it->revents & POLLIN) // jeśli klient wysyła dane
			{
				...
				{
					// jeśli wszystko ok, przejdź do kolejnego połączenia
			...
			else ++it; // w przeciwnym wypadku przejdź do kolejnego klienta
		}
	}
}
...
bool SOCK_NET::Server::connectWithClient()
{
	...
	ClientData cData = {
		clientIp, // ip klienta
		clientsCount, // licznik (numer klienta)
...
bool SOCK_NET::Server::readHeader(const SOCKET& cSock, SHORT& cEv)
{
	...
	for (int i = 0; i < SOCK_NET::Lib::MAX_BUFF; i++) currClient.header += buffer[i];

	if (currClient.header.size() > 0) // jeśli header zawiera jakiekolwiek znaki
	{
		const size_t hLenght = currClient.header.size(); // długość nagłówka
		const std::string EOL = currClient.header.substr(hLenght - 1, hLenght); // koniec danych
		// jeśli znaleziono w buforze znak końca headera lub znak nowej linii lub powrót karetki
		if (EOL == SOCK_NET::Lib::SEND_INDC || EOL == "\n" || EOL == "\r")
		{
			this->logDetails("INFO", "Odczytano naglowek od klienta.", cSock, currClient);
			currClient.header.pop_back(); // usuwanie ostatniego znaku z headera
			...
			const std::string fileName = currClient.header.substr(delimPos + 1, hLenght); // nazwa pliku
...
bool SOCK_NET::Server::sendFileToClient(const SOCKET& cSock)
{
	...
	do
	{
		...
		{
			std::cerr << "[ERROR::SERVER]\tNieudane wyslanie ramki danych do klienta.\n";
			return false;
		}
...
{
	std::cout << "[SERVER::" << type << "]\t[IP: " << cData.address << ", fd: " << cSock
		<< ", i: " << cData.clNr << "]: " << mess << "\n";
}
